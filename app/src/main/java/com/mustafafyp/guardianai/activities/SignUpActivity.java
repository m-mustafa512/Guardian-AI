...
import com.mustafafyp.guardianai.dialogfragments.ConfirmationDialogFragment;
import com.mustafafyp.guardianai.dialogfragments.GoogleChildSignUpDialogFragment;
...
import com.mustafafyp.guardianai.interfaces.OnConfirmationListener;
import com.mustafafyp.guardianai.interfaces.OnGoogleChildSignUp;
import com.mustafafyp.guardianai.models.Child;
import com.mustafafyp.guardianai.models.Parent;
...
public class SignUpActivity extends AppCompatActivity
		implements OnConfirmationListener, OnGoogleChildSignUp {
	...
	private TextView txtParentBtn, txtChildBtn;
	...
	// ❌ REMOVED: async state flag (unreliable)
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		txtParentBtn = findViewById(R.id.txt_parent_account);
		txtChildBtn = findViewById(R.id.txt_child_account);

		txtSignUpEmail = findViewById(R.id.txtSignUpEmail);
		txtParentEmail = findViewById(R.id.txtParentEmail);

		// ❌ OLD LOGIC (ASYNC BUG) — COMMENTED OUT
        /*
        txtParentEmail.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable editable) {
                Query query = databaseReference.child("parents")
                        .orderByChild("email")
                        .equalTo(txtParentEmail.getText().toString().toLowerCase());
        ...
        */
		...
		if (!parent) {
			...
			txtChildBtn.setVisibility(View.VISIBLE);
			txtParentBtn.setVisibility(View.GONE);
		}
		...
		btnSignUp.setOnClickListener(v -> {
			...
			// ✅ NEW LOGIC: validate parent at button click
			if (parent) {
				signUp(email, password);
			} else {
				validateParentAndSignUp(email, password);
			}
		});

		btnLogin = findViewById(R.id.txtSignIN);
		btnLogin.setOnClickListener(view ->
				startActivity(new Intent(SignUpActivity.this, LoginActivity.class)));
	}

	// ✅ NEW METHOD — FIXES CHILD REGISTRATION
	private void validateParentAndSignUp(String email, String password) {

		String parentEmail = txtParentEmail.getText().toString().toLowerCase();

		if (!Validators.isValidEmail(parentEmail)) {
			txtParentEmail.setError(getString(R.string.enter_valid_email));
			txtParentEmail.requestFocus();
			return;
		}

		DatabaseReference parentsRef = databaseReference.child("parents");

		parentsRef.orderByChild("email")
				.equalTo(parentEmail)
				.addListenerForSingleValueEvent(new ValueEventListener() {
					...
					public void onDataChange(@NonNull DataSnapshot snapshot) {
						if (snapshot.exists()) {
							...
							// ❌ Parent not found
							txtParentEmail.setError(
									getString(R.string.this_email_isnt_registered_as_parent)
							);
							txtParentEmail.requestFocus();
						}
					}
						...
						if (task.isSuccessful()) {
							...
							switch (errorCode) {
								...
								case "ERROR_EMAIL_ALREADY_IN_USE":
									txtSignUpEmail.setError(
											getString(R.string.email_is_already_in_use));
									break;
								case "ERROR_WEAK_PASSWORD":
									txtSignUpPassword.setError(
											getString(R.string.weak_password));
									break;
								default:
									Toast.makeText(this,
											getString(R.string.sign_up_falied),
											Toast.LENGTH_SHORT).show();
							}
						}
					});
	...
	// ⬇️ REST OF FILE UNCHANGED (safe & correct)
	// addUserToDB(), uploadProfileImage(), Google sign-in, validators, etc.
...
private void signUpRoutine(String parentEmail) {
	...
	private boolean isValid() {
		...
		if (!parent) {
			if (!Validators.isValidEmail(txtParentEmail.getText().toString().toLowerCase())) {
				txtParentEmail.setError(getString(R.string.enter_valid_email));
				txtParentEmail.requestFocus();
				return false;
			}

			// ❌ OLD ASYNC CHECK — REMOVED
    /*
    if (!validParent) {
